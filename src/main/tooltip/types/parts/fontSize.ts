import { getFontSizeData } from '../../../../shared/helpers';
import { createTooltipTextNode } from '../../../helper';

export default async function fontSize(
  node,
  parent,
  { fontColor = '', fontSize = 0 }
) {
  if (node?.fontSize) {
    const fonSizeData = await getFontSizeData(node);
    ...
    textNode.characters += `${fonSizeData.join(', ')}`;
